...
function displayRequest(webrequest) {
  console.log('====displayRequest=======', webrequest);
...
function displayResponse(webrequest) {
  console.log('====displayResponse=======', webrequest);
  ...
  if (webrequest.responseHeaders) {
    ...
    tabcontent.appendChild(createTabData('Headers_' + randId, formatHeaders(webrequest.responseHeaders, webrequest.responseBody), 'WebResheader'));
  ...
  if (webrequest.responseBody) {
    var name = "nopadding";
    added = name;
    tablist.appendChild(createtabheader('WebResBody_' + randId, 'Body', name));
    tabcontent.appendChild(createTabData('WebResBody_' + randId, formatBody(webrequest.responseBody), name));
  }
  if (added) {
    requestInfo.appendChild(tablist);
    requestInfo.appendChild(tabcontent);
...
function formatBody(resBody) {
  if (resBody && resBody.length) {
    var newNode = document.createElement('pre');
    newNode.textContent = JSON.stringify(JSON.parse(resBody), undefined, 2);
    return newNode;
  }
};
